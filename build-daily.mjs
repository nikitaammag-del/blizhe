...
async function buildPrompts(seen) {
  ...
  const ranked = rows.map(r => ({ title: r[0].trim(), text: r[1].trim(), score: scorePrompt(r[1], r[2]) + (OURS.test(r[0] + ' ' + r[1].slice(0, 300)) ? 3 : 0) })).filter(x => x.text.length <= 1200 && !seen.has(norm(x.text))).sort((a, b) => b.score - a.score || a.title.localeCompare(b.title));
  ...
  if (HAS_LLM && items.length) { try {
    ...
    items = items.map((it, i) => (tr[i] && cyr(tr[i].title) >= 0.5 && cyr(tr[i].text.replace(/\$\{[^}]*\}|\{\{[^}]*\}\}/g, '')) >= 0.4) ? { ...it, title: tr[i].title, text: tr[i].text, lang: 'ru', src: it.src + ', перевод: ' + LLM_NAME } : it);
...
async function buildBooks(seen) {
  ...
  const ordered = [...ranked.filter(b => cyr(b.t) >= 0.5), ...ranked.filter(b => cyr(b.t) < 0.5)].slice(0, 15);
